...
btnRoll.addEventListener('click', function () {
	// implementing playing state
	if (playing) {
		// 1. Gen random dice roll note - dice is inside funciton as to roll with each click*
		const dice = Math.floor(Math.random() * 6) + 1;

		// 2. Display dice roll to page
		diceEl.classList.remove('hidden');
		diceEl.src = `dice-${dice}.png`;

		// 3. If rolled a '1' switch to next player
		if (dice !== 1) {
			// Add dice roll to current score
			currentScore += dice;
			document.getElementById(`current--${activePlayer}`).textContent =
				currentScore; // this does it dynamically.
		} else {
...
btnNew.addEventListener('click', function () {
	...
	if (document.querySelector('.player--0').classList.contains('player--active') && document.querySelector('.player--1').classList.contains('player--active')) {
